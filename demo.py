...
def detectAmbulance():
    try:
        ...
        img = cv2.imread(filename, 0)  # Traffic image
        template = cv2.imread('images/ambulance_template.jpeg', 0)  # Template image
        ...
        if template.shape[0] > img.shape[0] or template.shape[1] > img.shape[1]:
            scale_height = img.shape[0] / template.shape[0]
            scale_width = img.shape[1] / template.shape[1]
        ...
        plt.imshow(result, cmap='hot')  # Visualizing the match results
        plt.colorbar()

        plt.show()

        threshold = 0.6  # Can adjust based on your test accuracy
...
detect_button = Button(main, text="Detect Ambulance", command=detectAmbulance)
detect_button.place(x=50, y=400)
